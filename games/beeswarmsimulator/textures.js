(function () {
	window.textures_default = function () {
		let data = new Uint8Array((512 * 512) << 2),
			smoothstep = (a, b, t) => (b - a) * t * t * (3 - (t + t)) + a,
			hash2 = (x, y) => {
			...
			noise_base = (x, y) => {
				let _x = flr(x),
					...
					rt = hash2(_x + 1, _y),
					rb = hash2(_x + 1, _y + 1),
					l = smoothstep(lt, lb, y - _y),
					r = smoothstep(rt, rb, y - _y);
		...
		for (let i = 0, l = data.length; i < l; i += 4) {
			let x = (i >> 2) % 512,
				y = (i >> 2) * inv_h,
				v = (0.5 - noise2(x * 0.125 + 205, y * 0.125 - 323) * 0.5) * 255;
			data[i + 3] = v;
		}
		data[3] = 0;
		return data;
	...
	window.textures_decals = function (tex_ctx) {
		...
		let matrixStack = [],
			...
			vertex = function () {
				if (firstInPath) tex_ctx.moveTo(...arguments) + (firstInPath = 0);
				else tex_ctx.lineTo(...arguments);
			},
			bezierVertex = function () {
				tex_ctx.bezierCurveTo(...arguments);
			},
			endShape = () => {
				tex_ctx.fill();
				tex_ctx.stroke();
		...
		tex_ctx.lineTo(32, 5);
		tex_ctx.fill();
		for (let i = 0.18; i < Math.PI; i += Math.PI / 8) ellipse(Math.cos(i) * 40, Math.sin(i) * 30 + 15, 6, 6);
		rotate(-5);
		translate(0, 175);
		scale(0.5);
		tex_ctx.fillStyle = "rgb(17, 32, 43)";
