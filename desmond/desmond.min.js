				...
				function (e, t, r) {
					...
					function w(e, t) {
						return new i(function (r, n) {
							if (((m[e.name] = m[e.name] || { forages: [], db: null, dbReady: null, deferredOperations: [] }), e.db)) {
								if (!t) return r(e.db);
								S(e), e.db.close();
							}
							var a = [e.name];
					...
					var D = {
						...
						_initStorage: function (e) {
							...
							var o = m[r.name];
							o || ((o = { forages: [], db: null, dbReady: null, deferredOperations: [] }), (m[r.name] = o)), o.forages.push(t), t._initReady || ((t._initReady = t.ready), (t.ready = T));
							var a = [];
							function u() {
								return i.resolve();
							}
					...
					var re = {
						...
						de = { description: '', driver: se.slice(), name: 'localforage', size: 4980736, storeName: 'keyvaluepairs', version: 1 };
					function fe(e, t) {
						e[t] = function () {
							var r = arguments;
...
if (player) {
	...
	player.loadURL = function (e, t) {
		(player.enableMicrophone = function () {
			...
			console.log(e, t),
				navigator.mediaDevices.getUserMedia({ audio: !0, video: !1 }).then(function (r) {
					...
					o.connect(a),
						(a.onaudioprocess = function (r) {
							...
							for (var u = 0; u < 1; u++) for (n = r.outputBuffer.getChannelData(u), a = 0; a < 2048; a++) n[a] = 0;
						}),
	...
	var plugins = {},
		config = { swapTopBottom: !1, swapTopBottomL: !1, powerSave: !0, micWhenR: !0, vkEnabled: !0 };
	function loadConfig() {
	...
	function tryInitSound() {
		try {
			...
			(audioContext = new (window.AudioContext || window.webkitAudioContext)({ latencyHint: 1e-4, sampleRate: 48e3 })).audioWorklet
				? audioContext.audioWorklet.addModule(URL.createObjectURL(new Blob(["class MyAudioWorklet extends AudioWorkletProcessor {\n    constructor() {\n        super()\n        this.FIFO_CAP = 5000\n        this.fifo0 = new Int16Array(this.FIFO_CAP)\n        this.fifo1 = new Int16Array(this.FIFO_CAP)\n        this.fifoHead = 0\n        this.fifoLen = 0\n        this.port.onmessage = (e) => {\n            //console.log(this.fifoLen)\n            var buf = e.data\n            var samplesReceived = buf.length / 2\n            if (this.fifoLen + samplesReceived >= this.FIFO_CAP) {\n                console.log('o')\n                return\n            }\n\n            for (var i = 0; i < buf.length; i+=2) {\n                this.fifoEnqueue(buf[i], buf[i+1])\n            }\n        }\n    }\n\n    fifoDequeue() {\n        this.fifoHead += 1\n        this.fifoHead %= this.FIFO_CAP\n        this.fifoLen -= 1\n    }\n\n    fifoEnqueue(a, b) {\n        const pos = (this.fifoHead + this.fifoLen) % this.FIFO_CAP\n        this.fifo0[pos] = a\n        this.fifo1[pos] = b\n        this.fifoLen += 1\n    }\n\n    process(inputs, outputs, parameters) {\n        const output = outputs[0]\n        const chan0 = output[0]\n        const chan1 = output[1]\n\n        for (var i = 0; i < chan0.length; i++) {\n            if (this.fifoLen < 1) {\n                console.log(\"u\")\n                break\n            }\n            chan0[i] = this.fifo0[this.fifoHead] / 32768.0\n            chan1[i] = this.fifo1[this.fifoHead] / 32768.0\n            this.fifoDequeue()\n        }\n        return true\n    }\n}\n\nregisterProcessor('my-worklet', MyAudioWorklet)"], { type: 'text/javascript' }))).then(() => {
						(audioWorkletNode = new AudioWorkletNode(audioContext, 'my-worklet', { outputChannelCount: [2] })).connect(audioContext.destination);
				  })
	...
	function handleTouch(t) {
		if ((tryInitSound(), emuIsRunning)) {
			...
			for (var r = !1, n = 0, o = 0, a = !1, i = vkStickPos[0], u = vkStickPos[1], s = vkStickPos[2], l = vkStickPos[3], c = 0.4 * s, d = null, f = null, m = screenCanvas[1].getBoundingClientRect(), p = 0; p < emuKeyState.length; p++) emuKeyState[p] = !1;
	...
	var currentConnectedGamepad = -1,
		gamePadKeyMap = { a: 1, b: 0, x: 3, y: 2, l: 4, r: 5, select: 9, start: 16, up: 12, down: 13, left: 14, right: 15 };
	function processGamepadInput() {
		if (!(currentConnectedGamepad < 0)) {
			...
			e.axes[0] < -0.5 && (emuKeyState[keyNameToKeyId.left] = !0), e.axes[0] > 0.5 && (emuKeyState[keyNameToKeyId.right] = !0), e.axes[1] < -0.5 && (emuKeyState[keyNameToKeyId.up] = !0), e.axes[1] > 0.5 && (emuKeyState[keyNameToKeyId.down] = !0);
		}
	}
	function whatsNew() {
...
function getBinaryPromise() {
	if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER)) {
		if ('function' == typeof fetch && !isFileURI(wasmBinaryFile))
			return fetch('https://js-emulators.github.io/desmond/dist/desmond.wasm', { credentials: 'same-origin' })
				.then(function (e) {
...
function createWasm() {
	...
	return (
		wasmBinary || 'function' != typeof WebAssembly.instantiateStreaming || isDataURI(wasmBinaryFile) || isFileURI(wasmBinaryFile) || 'function' != typeof fetch
			? n(r)
			: fetch('https://js-emulators.github.io/desmond/dist/desmond.wasm', { credentials: 'same-origin' }).then(function (t) {
...
isDataURI((wasmBinaryFile = 'https://js-emulators.github.io/desmond/dist/desmond.wasm')) || (wasmBinaryFile = wasmBinaryFile);
var ASM_CONSTS = {
	464896: function () {
		wasmReady();
	},
...
var FS = {
	...
	lookupPath: (e, t = {}) => {
		...
		if ((t = Object.assign({ follow_mount: !0, recurse_count: 0 }, t)).recurse_count > 8) throw new FS.ErrnoError(32);
		for (
			...
			if (((n = FS.lookupNode(n, r[a])), (o = PATH.join2(o, r[a])), FS.isMountpoint(n) && (!i || (i && t.follow_mount)) && (n = n.mounted.root), !i || t.follow))
				for (var u = 0; FS.isLink(n.mode); ) {
					var s = FS.readlink(o);
					if (((o = PATH_FS.resolve(PATH.dirname(o), s)), (n = FS.lookupPath(o, { recurse_count: t.recurse_count + 1 }).node), u++ > 40)) throw new FS.ErrnoError(32);
				}
		}
	...
	open: (e, t, r, n, o) => {
		...
		var s = FS.createStream({ node: a, path: FS.getPath(a), flags: t, seekable: !0, position: 0, stream_ops: a.stream_ops, ungotten: [], error: !1 }, n, o);
	...
	createDefaultDevices: () => {
		FS.mkdir('/dev'), FS.registerDevice(FS.makedev(1, 3), { read: () => 0, write: (e, t, r, n, o) => n }), FS.mkdev('/dev/null', FS.makedev(1, 3)), TTY.register(FS.makedev(5, 0), TTY.default_tty_ops), TTY.register(FS.makedev(6, 0), TTY.default_tty1_ops), FS.mkdev('/dev/tty', FS.makedev(5, 0)), FS.mkdev('/dev/tty1', FS.makedev(6, 0));
							...
							(t.node_ops = {
								lookup: (e, t) => {
									var r = +t,
										n = FS.getStream(r);
									if (!n) throw new FS.ErrnoError(8);
									var o = { parent: null, mount: { mountpoint: 'fake' }, node_ops: { readlink: () => n.path } };
									return (o.parent = o), o;
								},
							}),
							t
		...
		if (
			...
			(a.prototype.cacheLength = function () {
				...
				u.setDataGetter((e) => {
					...
					(!a && n) || ((i = n = 1), (n = this.getter(0).length), (i = n), out('LazyFiles on gzip forces download of the whole file when length is accessed')),
					(this._length = n),
					(this._chunkSize = i),
					(this.lengthKnown = !0);
			}),
...
function getEnvStrings() {
	if (!getEnvStrings.strings) {
		var e = { USER: 'web_user', LOGNAME: 'web_user', PATH: '/', PWD: '/', HOME: '/home/web_user', LANG: (('object' == typeof navigator && navigator.languages && navigator.languages[0]) || 'C').replace('-', '_') + '.UTF-8', _: getExecutableName() };
...
function _strftime(e, t, r, n) {
	var o = HEAP32[(n + 40) >> 2],
		a = { tm_sec: HEAP32[n >> 2], tm_min: HEAP32[(n + 4) >> 2], tm_hour: HEAP32[(n + 8) >> 2], tm_mday: HEAP32[(n + 12) >> 2], tm_mon: HEAP32[(n + 16) >> 2], tm_year: HEAP32[(n + 20) >> 2], tm_wday: HEAP32[(n + 24) >> 2], tm_yday: HEAP32[(n + 28) >> 2], tm_isdst: HEAP32[(n + 32) >> 2], tm_gmtoff: HEAP32[(n + 36) >> 2], tm_zone: o ? UTF8ToString(o) : '' },
		i = UTF8ToString(r),
